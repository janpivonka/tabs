...
export function SidebarFooter({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        <span className="text-base">💾</span> Synchronizovat {selectedCount > 0 ? selectedCount : "vše"}
      </button>

      <button
        onClick={onDeleteSelected}
        className="w-full bg-red-500 hover:bg-red-600 text-white py-2.5 px-3 rounded-xl flex items-center justify-center gap-2 text-xs font-black transition-all shadow-lg shadow-red-100 uppercase tracking-wider"
      >
        <span className="text-base">🗑️</span> Smazat {selectedCount > 0 ? selectedCount : "vše"}
      </button>
